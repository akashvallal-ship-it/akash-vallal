...
$functions"""
    ...
    # Backwards compatibility: provide run_single_command for callers expecting it
    async def run_single_command(
        self, code_blocks: List[CodeBlock], cancellation_token: CancellationToken
    ) -> CommandLineCodeResult:
        """Compatibility wrapper around execute_code_blocks.

        Some callers (older versions) call `run_single_command`. Provide a thin wrapper
        that forwards to the newer `execute_code_blocks` implementation.
        """
        return await self.execute_code_blocks(code_blocks, cancellation_token)
